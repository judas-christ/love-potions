defmodule Linq do
  defmacro from({:in, _context, [{what, _, nil}, {:__aliases__, _aliases, [from_where]}]}, opts \\ []) do
    # IO.inspect ast
    # IO.inspect opts
    ...
    quote do
      "select #{unquote(select_clause)} from #{:"Elixir.#{unquote(from_where)}".table_name} as #{unquote(table_alias)}#{unquote(where_clause)}"
    end
  end
  ...
  def get_struct_def({:__block__, _, ast}) do
    for line <- ast, {:field, _field_line, [field_name, _field_type | _opts]} = line, into: [] do
      # TODO: get default from opts
      {field_name, nil}
    end
  end
  def get_struct_def({:field, _, [field_name, _field_type | _opts]}) do
    [{field_name, nil}]
  end
  ...
  def get_where_clauses({_atom, _, nil}, table_alias) do
    table_alias
  end
